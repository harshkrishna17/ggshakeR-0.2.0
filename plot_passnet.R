...
#' @param colour_scale Colour of higher end of xT colour scale. Default set to "#E74C3C"
...
plot_passnet <- function(eventData, dataType = "statsbomb", 
                         team_name, colour_scale = "#E74C3C", subtitlePlot = "", theme = "") {
  ...
  if(dataType == "statsbomb") {
    ...
    if(subtitlePlot == "") {
      subtitlePlot <- paste0("Till First Substitution (", min, "')")
    }
    ...
    plot_passnet <- ggplot() +
      ...
      scale_fill_gradient(low = "black", high = colour_scale) +
